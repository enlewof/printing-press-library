...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool, mutationIntent bool) (json.RawMessage, int, error) {
	...
	// Concur splits its surface across host families: the modern v4/SCIM
	// host (c.BaseURL, e.g. www-us2.api.concursolutions.com) and several
	// legacy v1.1/v2.0 APIs (Expense Delegators, Travel Profile) documented
	// against a different literal host (www.concursolutions.com/api/...).
	// A resource whose path is already an absolute URL targets its own host
	// directly instead of being concatenated onto c.BaseURL.
	targetURL := path
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "concur-pp-cli auth set-token <token>")
}
