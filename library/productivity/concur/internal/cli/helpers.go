...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'concur-pp-cli doctor' to check auth status."+
	...
	// Must be checked before the generic HTTP 404 case since both match "HTTP 404".
	case strings.Contains(msg, "HTTP 404") && strings.Contains(msg, "No static resource") && strings.Contains(msg, "/expenses"):
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Run 'concur-pp-cli auth login --chrome' to refresh browser-session credentials."))
	}
	if len(trimmed) == 0 {
